    ...
    [TAPDataManager getRoomListSuccess:^(NSArray *resultArray) {
        if ([resultArray count] == 0) {
            ...
            if ([userID isEqualToString:@""]) {
                NSError *localizedError = [NSError errorWithDomain:NSLocalizedString(@"Current active user is not found....", @"") code:999 userInfo:@{@"message": NSLocalizedString(@"Current active user is not found...", @"")}];
                failure(localizedError);
            }
