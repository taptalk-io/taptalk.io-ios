...
//    self.statusIconImageView.alpha = 0.0f;
    ...
    self.mapView.layer.maskedCorners = kCALayerMaxXMaxYCorner | kCALayerMinXMinYCorner | kCALayerMinXMaxYCorner;
...
- (IBAction)forwardCheckmarkButtonDidTapped:(id)sender {
    if ([self.delegate respondsToSelector:@selector(myLocationCheckmarkDidTapped:)]) {
        [self.delegate myLocationCheckmarkDidTapped:self.message];
    }
}
...
- (void)setForwardData:(TAPForwardFromModel *)forwardData {
    ...
    NSString *initialAppendedFullnameString = [NSString stringWithFormat:@"From: ", nil, [TAPUtil currentBundle], @""];
...
- (void)showCheckMarkIcon:(BOOL)isShow {
    if(isShow){
        self.checkMarkIconImageView.alpha = 1.0f;        self.bubbleViewLongPressGestureRecognizer.enabled = NO;
