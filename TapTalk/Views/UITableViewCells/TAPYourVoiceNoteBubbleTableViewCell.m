//
//  TAPYourVoiceNoteBubbleTableViewCell.m
//  TapTalk
//
...
@implementation TAPYourVoiceNoteBubbleTableViewCell
...
- (void)awakeFromNib {
    ...
    [self showFileBubbleStatusWithType:TAPYourVoiceNoteBubbleTableViewCellStateTypeNotDownloaded];
    ...
    self.audioSlider.maximumTrackTintColor = [TAPUtil getColor:@"FFCB99"];
    self.audioSlider.minimumTrackTintColor = [TAPUtil getColor:@"FF7E00"];
    self.audioSlider.thumbTintColor = [TAPUtil getColor:@"FF7E00"];
...
- (IBAction)playerAudioSliderDidChanged:(id)sender {
    if ([self.delegate respondsToSelector:@selector(yourVoiceNoteBubblePlayerSliderDidChange:message:)]) {
        [self.delegate yourVoiceNoteBubblePlayerSliderDidChange:self.audioSlider.value message:self.message];
    }
}
- (IBAction)playerAudioSliderDidEnd:(id)sender {
    if ([self.delegate respondsToSelector:@selector(yourVoiceNoteBubblePlayerSliderDidEnd)]) {
        [self.delegate yourVoiceNoteBubblePlayerSliderDidEnd];
    }
}

- (IBAction)playerAudioSliderDidEnd2:(id)sender {
    if ([self.delegate respondsToSelector:@selector(yourVoiceNoteBubblePlayerSliderDidEnd)]) {
        [self.delegate yourVoiceNoteBubblePlayerSliderDidEnd];
    }
}
     ...
     if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        if (recognizer.state == UIGestureRecognizerStateChanged) {
            ...
            if (translation.x > 50.0f) {
                if ([self.delegate respondsToSelector:@selector(yourVoiceNoteBubbleDidTriggerSwipeToReplyWithMessage:)]) {
                    [self.delegate yourVoiceNoteBubbleDidTriggerSwipeToReplyWithMessage:self.message];
                }
            }
...
- (void)setBubbleCellStyle {
    ...
    self.progressContainerView.backgroundColor = [[[TAPStyleManager sharedManager] getComponentColorForType:TAPComponentColorLeftBubbleQuoteBackground] colorWithAlphaComponent:0.2f];
...
- (void)setMessage:(TAPMessageModel *)message {
    ...
    self.bubbleLabel.text = @"saxxasxasxasxasxasxasxsaxasxasxsaxsaxxasxasxxsaxs";
...
- (IBAction)senderProfileImageButtonDidTapped:(id)sender {
    if ([self.delegate respondsToSelector:@selector(yourVoiceNoteBubbleDidTappedProfilePictureWithMessage:)]) {
        [self.delegate yourVoiceNoteBubbleDidTappedProfilePictureWithMessage:self.message];
    }
}


- (IBAction)forwardCheckmarkButtonDidTapped:(id)sender {
    if ([self.delegate respondsToSelector:@selector(yourVoiceNoteCheckmarkDidTapped:)]) {
        [self.delegate yourVoiceNoteCheckmarkDidTapped:self.message];
    }
}

- (IBAction)replyButtonDidTapped:(id)sender {
    if ([self.delegate respondsToSelector:@selector(yourVoiceNoteReplyDidTapped:)]) {
        [self.delegate yourVoiceNoteReplyDidTapped:self.message];
    }
}

- (IBAction)retryDownloadButtonDidTapped:(id)sender {
    if ([self.delegate respondsToSelector:@selector(yourVoiceNoteRetryDownloadButtonDidTapped:)]) {
        [self.delegate yourVoiceNoteRetryDownloadButtonDidTapped:self.message];
    }
}

- (IBAction)downloadButtonDidTapped:(id)sender {
    if ([self.delegate respondsToSelector:@selector(yourVoiceNoteDownloadButtonDidTapped:)]) {
        [self.delegate yourVoiceNoteDownloadButtonDidTapped:self.message];
    }
}

- (IBAction)doneDownloadButtonDidTapped:(id)sender {
    if ([self.delegate respondsToSelector:@selector(yourVoiceNoteOpenFileButtonDidTapped:)]) {
        [self.delegate yourVoiceNoteOpenFileButtonDidTapped:self.message];
    }
}
...
- (IBAction)cancelButtonDidTapped:(id)sender {
    if ([self.delegate respondsToSelector:@selector(yourVoiceNoteCancelButtonDidTapped:)]) {
        [self.delegate yourVoiceNoteCancelButtonDidTapped:self.message];
    }
}

- (void)handleBubbleViewTap:(UITapGestureRecognizer *)recognizer {
    if ([self.delegate respondsToSelector:@selector(yourVoiceNoteBubbleViewDidTapped:)]) {
        [self.delegate yourVoiceNoteBubbleViewDidTapped:self.message];
    }
}

- (void)handleBubbleViewLongPress:(UILongPressGestureRecognizer *)recognizer {
    if(recognizer.state = UIGestureRecognizerStateEnded) {
        if ([self.delegate respondsToSelector:@selector(yourVoiceNoteBubbleLongPressedWithMessage:)]) {
            [self.delegate yourVoiceNoteBubbleLongPressedWithMessage:self.message];
        }
    }
}
...
- (IBAction)quoteButtonDidTapped:(id)sender {
    if ([self.delegate respondsToSelector:@selector(yourVoiceNoteQuoteViewDidTapped:)]) {
        [self.delegate yourVoiceNoteQuoteViewDidTapped:self.message];
    }
}
...
- (void)showDownloadedState:(BOOL)isShow {
    ...
    if (isShow) {
        [self showFileBubbleStatusWithType:TAPYourVoiceNoteBubbleTableViewCellStateTypeDoneDownloadedUploaded];
    }
    else {
        [self showFileBubbleStatusWithType:TAPYourVoiceNoteBubbleTableViewCellStateTypeNotDownloaded];
    }
...
- (void)animateFailedDownloadFile {
    ...
    [self showFileBubbleStatusWithType:TAPYourVoiceNoteBubbleTableViewCellStateTypeRetry];
}
...
- (void)showFileBubbleStatusWithType:(TAPYourVoiceNoteBubbleTableViewCellStateType)type {
    ...
    if (type == TAPYourVoiceNoteBubbleTableViewCellStateTypeDoneDownloadedUploaded) {
        self.cancelView.alpha = 0.0f;
        self.downloadView.alpha = 0.0f;
        self.doneDownloadView.alpha = 1.0f;
        self.retryDownloadView.alpha = 0.0f;
    }
    else if (type == TAPYourVoiceNoteBubbleTableViewCellStateTypeNotDownloaded) {
        self.cancelView.alpha = 0.0f;
        self.downloadView.alpha = 1.0f;
        self.doneDownloadView.alpha = 0.0f;
        self.retryDownloadView.alpha = 0.0f;
    }
    else if (type == TAPYourVoiceNoteBubbleTableViewCellStateTypeDownloading) {
        self.cancelView.alpha = 1.0f;
        self.downloadView.alpha = 0.0f;
        self.doneDownloadView.alpha = 0.0f;
        self.retryDownloadView.alpha = 0.0f;
    }
    else if (type == TAPYourVoiceNoteBubbleTableViewCellStateTypeRetry) {
        self.cancelView.alpha = 0.0f;
        self.downloadView.alpha = 0.0f;
        self.doneDownloadView.alpha = 0.0f;
